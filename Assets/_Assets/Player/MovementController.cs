...
public class MovementController : MonoBehaviour
{
    ...
    [SerializeField] float mMaxFallSpeed = 5f;
    ...
    [SerializeField] float mMaxMoveSpeed = 50f;
    ...
    private void UpdateTransform()
    {
        ...
        if (mHorizontalVelocity.sqrMagnitude > 0) // turn the body where we are looking
        {
    ...
    {
        if (mShouldTryJump && !mIsInAir) //try jump first
        {
        ...
        if (mCharacterController.isGrounded) //we are on ground, set velocity to small velocty going down
        {
        ...
        if (mVerticalVelocity.y > -mMaxFallSpeed) //free falling
        {
